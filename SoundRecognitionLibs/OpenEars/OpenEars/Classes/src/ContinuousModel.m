...
- (void) clearBuffer:(int16 *)audioDeviceBuffer {
    memset(audioDeviceBuffer, 0, kVadBufferSize * sizeof(int16)); 
}
...
- (int) getFirstFrameStateForBuffer:(int16 *)audioDeviceBuffer {
    ...
    if(audioDevice->recordData == 0) { // If we have looped back here in a suspended state, we exit.
        [self clearBuffer:audioDeviceBuffer];
...
- (void) detectSpeechInBuffer:(SInt16 *)audioDeviceBuffer usingSpeechData:(int32 *)speechData withSleepTime:(int)sleepTime {
    for (;;) { // This has been moved into its own loop so that more selective behavior is available.
        ...
        if(audioDevice->recordData == 0) { // If we have looped back here in a suspended state, we sleep and loop (or break if we need to).
            [self clearBuffer:audioDeviceBuffer];// If we aren't recording, erase the buffer so we never have a buffer with old data in it upon resuming.
            firstEntryIntoOuterLoopAfterResuming = 1; // Note that the next time this loop is entered without hitting this condition, it will be the first recognition after resuming (which is handled in a particular way to exclude bad noise values)
        ...
        if(firstEntryIntoOuterLoopAfterResuming == 1) {
            firstEntryIntoOuterLoopAfterResuming = 0;
            firstFrame = 1;
            [self clearBuffer:audioDeviceBuffer];
        ...
        if((*speechData = cont_ad_read(self.continuousListener, audioDeviceBuffer, kVadBufferSize,firstFrame)) == 0) {
            if([self shouldBreakForCondition:kConditionExitListeningLoopOrLanguageModelChangeRequest]) return;
            usleep(sleepTime);
            if([self shouldBreakForCondition:kConditionExitListeningLoopOrLanguageModelChangeRequest]) return;
            continue;
...
- (int) checkForEndOfSpeechForBuffer:(int16 *)audioDeviceBuffer andFirstFrame:(int)firstFrame withSpeechData:(int32 *)speechData andTimeStamp:(int32 *)timestamp {
    ...
    *speechData = cont_ad_read(self.continuousListener, audioDeviceBuffer, kVadBufferSize, firstFrame); // Read the available data.
    ...
    if (*speechData < 0) { // If a negative returns, that's an error.
        ...
        if (((self.continuousListener->read_ts - *timestamp) > (kSamplesPerSecond * self.secondsOfSilenceToDetect)) || self.longRecognition == TRUE) {
            if(self.longRecognition == TRUE) { // If we're here due to a long recognition we need to recalibrate as quickly as possible.
